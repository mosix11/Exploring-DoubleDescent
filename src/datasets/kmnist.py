...
class KMNIST(BaseClassificationDataset):
    ...
    def load_train_set(self):
        ...
        if self.is_distributed():
            if self.is_node_leader():
                _ = datasets.KMNIST(root=root, train=True, download=True)  # pre-download only
            dist.barrier()
            trainset = datasets.KMNIST(root=root, train=True, transform=self.train_transforms, download=False)
        else:
            trainset = datasets.KMNIST(root=root, train=True, transform=self.train_transforms, download=True)
    ...
    def load_test_set(self):
        ...
        if self.is_distributed():
            if self.is_node_leader():
                _ = datasets.KMNIST(root=root, train=False, download=True)  # pre-download only
            dist.barrier()
            testset = datasets.KMNIST(root=root, train=False, transform=self.val_transforms, download=False)
        else:
            testset = datasets.KMNIST(root=root, train=False, transform=self.val_transforms, download=True)
